...
from apca.common.io_utils import read_json  # noqa: E402
...
def main() -> int:
    ...
    if active("safety"):
        sh([PY, "apca/safety_audit.py"])

    if active("splits"):
        sh([PY, "apca/build_splits.py", "--bfcl-limit", str(args.bfcl_scan)])

    if active("canonical"):
        sh([PY, "apca/canonical_message_cache.py", "--freeze"], allow_fail=True)

    if active("calibration"):
        cmd = [PY, "apca/run_calibration.py", "--models", *args.candidates,
    ...
    if active("dev"):
        ...
        cmd = [PY, "apca/run_dev.py", "--models", *targets,
    ...
    if active("prerun_review"):
        ...
        sh([PY, "apca/run_dual_review.py", "--mode", "pre-run"], allow_fail=True)

    if active("confirmatory"):
        # Frozen fast path: frozen priors + program guard, no live per-turn reviewers.
        cmd = [PY, "apca/run_confirmatory.py", "--stage", "confirmatory",
    ...
    if active("integrity"):
        sh([PY, "apca/check_integrity.py",
            "--stage-file", str(paths.CONFIRMATORY / "confirmatory_episodes.jsonl")], allow_fail=True)

    if active("analyze"):
        sh([PY, "apca/analyze_confirmatory.py"])

    if active("confounder"):
        sh([PY, "apca/run_confounder_module.py", "--models", *targets,
            "--repeats", str(args.confounder_repeats), "--no-live-attacker"], allow_fail=True)

    if active("review"):
        sh([PY, "apca/run_dual_review.py", "--target", "300"], allow_fail=True)
